...
# 수심 데이터 처리 및 안전 경로 계획 함수들

def get_depth_data(lat, lon):
    """
    특정 위치의 수심 데이터를 가져오는 함수
    실제 구현시에는 GEBCO나 NOAA 등의 데이터베이스에서 수심 정보를 가져와야 함
    """
    try:
        # 예시 수심 데이터 (실제 구현 시 API나 데이터베이스 연동 필요)
        # 수심은 음수값으로 표현 (예: -50m는 수심 50m를 의미)
        depth_database = {
            # 예시 데이터: (위도, 경도): 수심
            (34.7, 126.3): -50,
            (34.8, 126.4): -30,
        ...
        # 가장 가까운 지점의 수심 반환
        closest_point = min(depth_database.keys(), 
                          key=lambda p: haversine(lat, lon, p[0], p[1]))
        return depth_database[closest_point]
    except Exception as e:
        print(f"수심 데이터 조회 오류: {e}")
        return None

def is_safe_depth(depth, min_safe_depth=-20, max_safe_depth=-100):
    """
    안전한 수심 범위인지 확인하는 함수
    Args:
        depth: 현재 수심 (음수값)
        min_safe_depth: 최소 안전 수심 (음수값)
        max_safe_depth: 최대 안전 수심 (음수값)
    """
    return min_safe_depth >= depth >= max_safe_depth

def check_water_area(lat, lon):
    """
    해당 위치가 바다인지 확인하는 함수
    실제 구현시에는 육지/바다 구분 데이터베이스 필요
    """
    try:
        # 예시 구현 (실제로는 지형 데이터베이스 사용 필요)
        depth = get_depth_data(lat, lon)
        return depth is not None and depth < 0
    except Exception as e:
        print(f"수역 확인 오류: {e}")
        return False

def modify_obstacle_map_with_depth(obstacle_map, lat_grid, lon_grid, min_safe_depth=-20):
    """
    장애물 맵에 수심 정보를 추가하는 함수
    """
    for i in range(len(lat_grid)):
        for j in range(len(lon_grid)):
            # 수심 데이터 확인
            depth = get_depth_data(lat_grid[i], lon_grid[j])
            
            # 육지이거나 안전하지 않은 수심인 경우 장애물로 처리
            if not check_water_area(lat_grid[i], lon_grid[j]) or \
               (depth is not None and not is_safe_depth(depth)):
                obstacle_map[i][j] = 1
    
    return obstacle_map

# 기존 create_obstacle_map 함수 수정
def create_obstacle_map_with_depth(lat_grid, lon_grid, obstacles, moving_obstacles, 
                                 ship_lat, ship_lon, sensing_radius,
                                 obstacle_radius=0.5, avoidance_cost=5):
    """
    수심 정보를 포함한 장애물 맵 생성
    """
    # 기존 장애물 맵 생성
    obstacle_map = create_obstacle_map(lat_grid, lon_grid, obstacles, 
                                     moving_obstacles, ship_lat, ship_lon,
                                     sensing_radius, obstacle_radius, 
                                     avoidance_cost)
    
    # 수심 정보 추가
    obstacle_map = modify_obstacle_map_with_depth(obstacle_map, lat_grid, lon_grid)
    
    return obstacle_map

def check_route_safety(route_lats, route_lons):
    """
    생성된 경로의 안전성 검사
    """
    for lat, lon in zip(route_lats, route_lons):
        if not check_water_area(lat, lon):
            return False
        
        depth = get_depth_data(lat, lon)
        if depth is not None and not is_safe_depth(depth):
            return False
...
def init_simulation():
    """시뮬레이션 초기 설정"""
    global initial_lat, initial_lon, destination_lat, destination_lon, ship_speed
    global total_time, time_step, num_steps, sensing_radius, obstacle_radius
    global ship_distance_traveled, reached_destination, current_waypoint_index
    global current_lat, current_lon, previous_lat, previous_lon

    # 시뮬레이션 설정
    if simulation_mode:
        initial_lat, initial_lon = 34.804074, 126.360247
        destination_lat, destination_lon = 33.501271, 126.447097
        ship_speed = 30  # km/h
    else:
        # 실제 GPS 데이터를 사용하여 초기 위치 설정
        initial_lat, initial_lon, initial_speed, initial_direction = get_gps_data()
        # 실시간 관제 화면에서 입력한 목적지를 입력한다(수정필요함)
...
def init_waypoints_and_obstacles(inputWaypoint):
    global waypoints, fixed_obstacles, moving_obstacles
    ...
    if simulation_mode:
        ...
        moving_obstacles = [{'lat': 34.85, 'lon': 128.85, 'direction': 0, 'speed': 0}]  # 예시로 하나만 표시
        # 나머지 29개 장애물도 동일한 형식으로 추가

def init_route():
    """초기 경로 생성"""
    global full_route_lats, full_route_lons, route_lats_interp, route_lons_interp
    global cumulative_distances, lat_min_plot, lat_max_plot, lon_min_plot, lon_max_plot
